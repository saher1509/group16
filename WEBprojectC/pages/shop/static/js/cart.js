...
const renderCart = () => {
    ...
    if (cartItems.length === 0) {
        ...
        // Update URL to indicate the cart is empty
        window.history.replaceState(null, null, "/cart?status=empty");
    } else {
        ...
        cartItems.forEach(item => {
            ...
            itemNames.push(encodeURIComponent(item.name)); // Store item names in URL-friendly format
        ...
        // Update URL to indicate the cart is full
        window.history.replaceState(null, null, `/cart?status=full`);
    }
